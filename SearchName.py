...
from hashtable import GameManager

class SearchNameScreen(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("GameMatcher – Search by Title")
        ...
        # draw an outer white frame with a black border
        pad_x, pad_y = 50, 50
        canvas.create_rectangle(
            pad_x, pad_y,
            1330 - pad_x, 750 - pad_y,
            fill="white",      # the inner fill
            outline="black",   # the border color
            width=2            # border thickness
        )

        # light-grey banner behind label+entry
        ...
        # buttons, moved down under banner
    ...
    def perform_search(self):
        ...
        recs = self.manager.get_recommendations_by_title(key)
